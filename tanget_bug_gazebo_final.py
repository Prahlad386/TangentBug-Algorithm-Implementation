...
class TBug(Node):
    ...
    def lidar_callback(self, msg: LaserScan):
        ...
        self.angle_min = msg.angle_min      # Usually -π (front=0°)
        self.angle_increment = msg.angle_increment  # ~0.0087 rad (~0.5° per ray)
        self.num_rays = len(self.lidar_ranges)
    ...
    def get_distance_at_angle(self, target_angle_rad):
        """Return distance at exact angle (handles wraparound)"""
        # Normalize angle to [angle_min, angle_min+2π]
        angle = (target_angle_rad - self.angle_min)
        index = int(angle / self.angle_increment)
        
        # Clamp to array bounds
        #index = max(0, min(index, self.num_rays - 1))
        return self.lidar_ranges[index]
    ...
    def update_obstacles(self):
        ...
        for i, dist in enumerate(self.lidar_ranges):
            if dist < 10.0:
                angle = math.radians(self.angle_min + i*math.degrees(self.angle_increment))
    ...
    def wall_follow_right(self):
        ...
        min_range = min(abs(self.lidar_ranges[:int(length/4)]))
        min_range_next = min(abs(self.lidar_ranges[int(length/8):int(3*length/8)]))
        index_for_angle = np.argmin(abs(self.lidar_ranges))
        angle = self.angle_min + index_for_angle * self.angle_increment
        print(min_range, min_range_next)
        ...
        if min_range > 2.0:
            print('DRIVE')
            self.action = 'DRIVE'
            self.publish_velocity(2.0, 0.0)
        
        if min_range_next > 4.0:
    ...
    def wall_follow_left(self):
        ...
        #     return
        length = len(self.lidar_ranges)
        min_range = min(abs(self.lidar_ranges[int(3*length/4):length]))
        min_range_next = min(abs(self.lidar_ranges[int(5*length/8):int(7*length/8)]))
        index_for_angle = np.argmin(abs(self.lidar_ranges))
        angle = self.angle_min + index_for_angle * self.angle_increment
        print(min_range, min_range_next)
        ...
        if self.state == 'SEEK_GOAL':
            ...
            if self.line_of_sight_clear():
                ...
                if tangent is not None:
                    self.best_tangent_point = self.push_tangent_perpendicular(tangent, side, self.TANGENT_OFFSET)
                    self.wall_side = side
                    ...
                    self.wall_side = 'RIGHT' #can be right also
                    self.dmin = dist_to_goal
